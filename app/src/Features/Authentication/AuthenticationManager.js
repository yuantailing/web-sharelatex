...
const AuthenticationManager = {
  authenticate(query, password, callback) {
    ...
    if (typeof password === 'object' && password.provider == 'github') {
      // GitHub OAuth
      return User.findOne(
        {
          $query: {
            thirdPartyIdentifiers: {
              $elemMatch: {
                providerId: 'github',
                externalUserId: password.id.toString()
              }
            }
          },
          $orderby: [['_id', 1]]
        ...
        (error, user) => {
          ...
          return request.get(
            {
              url:
                'https://api.github.com/orgs/thu-media/members/' +
                password.login,
              headers: {
                Authorization:
                  'token ' + Settings.cgservice.GITHUB_PERSONAL_ACCESS_TOKEN,
                'User-Agent': 'Sharelatex Community Plus'
              },
              json: true
            },
            function(e, r, body) {
              if (e) return callback(e)
              if (r.statusCode != 204) return callback(null, null)
    ...
    return request.post(
      ...
      (err, response, body) => {
        ...
        if (response.statusCode !== 200) return callback(null, null)
        ...
        if (data.error) {
          return User.findOne(query, (error, user) => {
            ...
            bcrypt.compare(password, user.hashedPassword, function(
              error,
              match
            ) {
              if (error) {
        ...
        return User.findOne(
          {
            $query: {
              thirdPartyIdentifiers: {
                $elemMatch: {
                  providerId: 'cgserver',
                  externalUserId: data.user_id.toString()
                }
              }
            },
            $orderby: [['_id', 1]]
